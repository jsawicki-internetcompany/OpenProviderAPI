...
class OpenProviderApi
{
    ...
    public function processRawReply (OpenProviderRequest $r) {
        if ($this->debug) {
            echo $r->getRaw() . "\n";
        }
        $msg = $r->getRaw();
        $str = $this->_send($msg);
        if (!$str) {
            throw new OpenProviderAPIException("Bad reply", 4004);
        }
        if ($this->debug) {
            echo $str . "\n";
        }
    ...
    public function process (OpenProviderRequest $r) {
        if ($this->debug) {
            echo $r->getRaw() . "\n";
        }
        ...
        return new OpenProviderReply($str);
    }
    /**
    ...
    static function encode($str)
    {
        ...
        $ret = htmlentities($str, ENT_QUOTES, self::$encoding);
    ...
    {
        return new OpenProviderRequest ($xmlStr);
    }
    static function createReply ($xmlStr = null)
    {
        return new OpenProviderReply ($xmlStr);
    }
    ...
    protected static function parseArray ($node)
    {
        ...
        foreach ($node->childNodes as $child) {
            $name = self::decode($child->nodeName);
            if ('item' !== $name) {
                throw new OpenProviderAPIException('Wrong message format', 4006);
            }
