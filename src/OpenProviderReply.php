...
namespace OpenProviderAPI;

use DOMDocument;
use OpenProviderAPI\Exception\OpenProviderAPIException;

class OpenProviderReply
{
    ...
    protected function _parseReply ($str = '')
    {
        ...
        $arr = OpenProviderApi::convertXmlToPhpObj($dom->documentElement);
        if ((!is_array($arr) && trim($arr) == '') ||
            $arr['reply']['code'] == 4005)
        {
            throw new OpenProviderAPIException("API is temporarily unavailable due to maintenance", 4005);
        }
    ...
    {
        return OpenProviderApi::encode($str);
    }
    ...
    {
        $dom = new DOMDocument('1.0', OpenProviderApi::$encoding);
        ...
        $descNode->appendChild(
            $dom->createTextNode(OpenProviderApi::encode($this->faultString))
        );
        $dataNode = $replyNode->appendChild($dom->createElement('data'));
        OpenProviderApi::convertPhpObjToDom($this->value, $dataNode, $dom);
        if (0 < count($this->warnings)) {
            $warningsNode = $replyNode->appendChild($dom->createElement('warnings'));
            OpenProviderApi::convertPhpObjToDom($this->warnings, $warningsNode, $dom);
        }
        $this->dom = $dom;
